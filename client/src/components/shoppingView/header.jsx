  ...
  return <nav className="flex flex-col mb-3 lg:mb-0 lg:items-center gap-6 lg:flex-row">
    {
      shoppingViewHeaderMenuItems.map((item) => (
        <Label onClick={() => handleNavigate(item)} className="text-sm font-medium cursor-pointer" key={item.id}>{item.label}</Label>
      ))
    }
  </nav>
}
  ...
  return (
    <div className="flex lg:items-center lg:flex-row flex-col gap-4">
      <Sheet open={openCartSheet} onOpenChange={() => setOpenCartSheet(false)}>
        ...
        <UserCartWrapper handleCheckoutPage={handleCheckoutPage} cartItems={cartItems && cartItems.items && cartItems.items.length > 0 ? cartItems.items : []} />
      </Sheet>
    ...
    <header className="sticky top-0 w-full border-b bg-background">
      <div className="flex h-16 items-center justify-between px-4 md:px-6">
        ...
        <Sheet>
          ...
          <SheetContent side="left" className="w-full max-w-xs overflow-y-scroll">
            <HeaderRightContent />
            <MenuItemes />
          </SheetContent>
        </Sheet>
